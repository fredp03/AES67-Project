...
// Sender doesn't need to bind to a specific port or join multicast group
// It just sends TO the multicast address:port
// The receiver (monitor) binds to the port and joins the multicast group
logger('RTP client configured to send to', aes67Multicast + ':5006');
...
//ptp sync timeout - modified to continue without PTP
// Skip PTP initialization entirely when monitor is running (port conflict)
setTimeout(function(){
	if(!ptpMaster){
		console.warn('Skipping PTP sync (monitor handles PTP). Using system time for RTP timestamps.');
		// Use a dummy PTP master ID
		ptpMaster = '00:00:00:00:00:00:00:00';
		// Start audio stream immediately
		logger('Starting SAP announcements and audio stream (no PTP sync).');
		rtAudio.start();
		sdp.start(addr, aes67Multicast, samplerate, audioChannels, encoding, streamName, sessID, sessVersion, ptpMaster);
	}
}, 100); // Reduced timeout since we're not waiting for PTP

//init PTP client - DISABLED to avoid port conflict with aes67-monitor
// The monitor already handles PTP on ports 319/320
...
let rtpSend = function(pcm){
	//convert L16 to L24 (signed 16-bit to signed 24-bit, left-justified)
	// RtAudio gives us SINT16 little-endian, we need L24 big-endian (3 bytes per sample)
	let samples = pcm.length / 2;
	let l24 = Buffer.alloc(samples * 3);
	
	for(let i = 0; i < samples; i++){
		// Read signed 16-bit sample (little-endian)
		let sample16 = pcm.readInt16LE(i * 2);
		// Convert to 24-bit by left-shifting 8 bits (puts 16-bit audio in upper bits)
		let sample24 = sample16 << 8;
		// Write as 24-bit big-endian (3 bytes)
		l24.writeIntBE(sample24, i * 3, 3);
	}
	...
	if(correctTimestamp){
		...
		try {
			...
			// PTP not available, use system time as fallback
			let now = Date.now();
			let timestampRTP = Math.floor((now / 1000) * samplerate) % 0x100000000;
			timestampCalc = Math.floor(timestampRTP / fpp)*fpp;
		}
	}
	...
	//send RTP packet to port 5006 (avoiding macOS MIDIServer on 5004-5005)
	client.send(rtpBuffer, 5006, aes67Multicast);	
	...
	try {
		...
		// Skip timestamp averaging if PTP not available
	}
